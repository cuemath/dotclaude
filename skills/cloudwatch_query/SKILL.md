...
allowed-tools: Bash(aws *), Bash(date *), Bash(sleep *)
...
- **Default time range:** last 4 weeks
...
1. **Parse arguments.** Determine:
   ...
   - **Time range:** Look for natural language like "last 7 days", "last 24 hours", "last 1 hour". Default to last 4 weeks.
...
```bash
sleep 2 && aws logs get-query-results --query-id '<query_id>' --output json
```
