...
```json
{
  ...
  "repos": {
    "backend": "/Users/akash/workspace/microservices",
    "frontend": "/Users/akash/workspace/frontend"
  }
...
### Slack Class Issue Detection → Delegate to `redshift-query` skill

After extracting the Slack thread content, scan ALL messages (parent + replies) for **classroom issue context**. If the thread contains BOTH of the following, set `redshift_event_query = true`:

1. **A teacher or student ID** — UUID patterns (e.g., `1f666feb-255b-11e9-815c-0624d341e5a7`), or explicit mentions like `teacher_id`, `student_id`, or context clues ("this teacher", "student faced issue")

2. **Time or class details** — timestamps ("3:00 PM IST", "15:00"), dates ("13th March", "2026-03-13"), class references (`class_id`, "during class"), or relative time ("today's class", "yesterday's class")

**If `redshift_event_query = true`:**

- Extract: `user_id` (UUID), `user_type` (teacher/student, default teacher if ambiguous), `class_date` (YYYY-MM-DD), `class_time_ist` (HH:MM:SS in IST)
- Include a Redshift event query step in the PLAN phase
- During TRIAGE (Phase 3), invoke the **`redshift-query` skill** with the extracted context. The redshift-query skill owns all Redshift connection logic, table schemas, query patterns, and classroom event analysis. Pass it a query like: `"What happened for <user_type> <user_id> during class on <class_date> at <class_time_ist> IST?"`
- Use the Redshift results as evidence in HYPOTHESIZE and VERIFY phases, same as any other data source
...
**For classroom/AV issues (`redshift_event_query = true`), also run:**

12. **Invoke the `redshift-query` skill** with the extracted `user_id`, `user_type`, `class_date`, and `class_time_ist`. The skill handles connection, table selection, query execution, and event analysis. Use the returned event data as triage evidence.
...
- Redshift Events: <if redshift_event_query=true: event count, error events found, session flow summary>
```
...
**Classroom/AV failures (Redshift event data available):**

- Use event data from the `redshift-query` skill to correlate failures across AV, PubNub, and worksheet loading
- Run additional Redshift queries via the skill if needed (e.g., query the other side — student events if you have teacher data, or vice versa)
...
| "Teacher/student had AV/network/class issue" | Invoke `redshift-query` skill for the user's session events during the class window        |
